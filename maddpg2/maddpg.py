...
class MADDPG:
    ...
    def update_policy(self):
        ...
        for agent in range(self.n_agents):
            ...
            next_Q = torch.empty(self.batch_size).type(FloatTensor)
            next_Q[non_final_mask] = self.critics_target[agent](
                non_final_next_states.view(-1, self.n_agents * self.n_states),
                non_final_next_actions.view(-1, self.n_agents * self.dim_act)
            ).squeeze()
            target_Q = next_Q * self.GAMMA + reward_batch[:, agent].unsqueeze(1) * self.scale_reward
    ...
    def select_action(self, states):
        actions = torch.zeros(self.n_agents, self.dim_act)
        for i in range(self.n_agents):
            state = states[i]
            actions[i, :] = self.actors[i](state)
        for i in range(self.n_agents):
            actions[i] += torch.tensor(np.random.randn(self.dim_act) * self.var[i] * 10).float()
            if self.episode_done > self.episodes_before_train and self.var[i] > 0.01:
                self.var[i] *= 0.9998
            if self.var[i] <= 0.01:
                self.var[i] = 0.01
            actions[i] = torch.clamp(actions[i], -20.0, 20.0)
        self.steps_done += 1
        return actions
        ...
        if torch.cuda.is_available():
            ...
            for i in range(len(self.critics)):
                self.critics[i].load_state_dict(
                    torch.load('critic' + str(i) + '.pth', map_location=torch.device('cpu')))
